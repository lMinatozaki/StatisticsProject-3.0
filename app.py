...
def graficar_distribucion_fisher(t, gl_error, ftab, fcalc):
    x = np.linspace(0, ftab + 5, 1000)
    y = f.pdf(x, t, gl_error)

    plt.figure(figsize=(10, 6))
    plt.plot(x, y, label="Distribucion F de Fisher", color="royalblue", lw=2)
    ...
    plt.axvline(fcalc, color='lawngreen', linestyle='dashed', label=f"FCalc = {round(fcalc, 4)}", lw=2, zorder=2)
...
def analisis_andeva():
    ...
    graficar_distribucion_fisher(GL_tratamiento, GL_error, FTab, Fcalc)
